...
2. Создать `AuthModule` с эндпоинтами:
   ...
   - `POST /api/auth/login` — вход email/пароль, возврат `access_token` (JWT)
   - `GET /api/auth/google` — редирект на Google OAuth
   - `GET /api/auth/google/callback` — обработка callback, возврат JWT
   - `GET /api/auth/me` — текущий пользователь (guard)
   - `POST /api/auth/logout` — логаут (на клиенте удаляем токен)
3. Реализовать `JwtAuthGuard` и декоратор `@CurrentUser()`
4. Настроить `ConfigModule` глобально для чтения `.env`
5. Написать unit-тесты для `AuthService`
...
5. Создать `FinanceAnalyticsModule`: `GET /api/finance/analytics` с параметрами `from`, `to` — возвращает: баланс, разбивку по категориям, динамику по дням, сравнение с предыдущим периодом, средние значения, топ категорий, крупнейшие транзакции, % сбережений
...
4. Реализовать кнопку "Войти через Google" — редирект на `GET /api/auth/google`; обработать callback-страницу `/auth/callback` для приёма JWT из query-параметра
5. Создать `apiClient` — экземпляр axios с базовым URL, интерцептором для добавления `Authorization: Bearer <token>` из стора
6. Создать хук `useCurrentUser` — при монтировании проверяет токен через `GET /api/auth/me`
7. Реализовать страницу `/profile`: email пользователя, кнопка выхода
...
4. Реализовать вложенность: подзадачи рендерятся под родителем с отступом; drag-and-drop для вложения (drop задачи на задачу делает её подзадачей)
...
2. Реализовать страницу/модал `/notes/[id]`: textarea для текстового содержимого (без форматирования), поле заголовка, выбор категории
...
2. Реализовать форму создания/редактирования записи (модал или отдельная страница): сумма, тип, категория, описание, дата; валюта пока только RUB (поле в DTO есть, UI не отображает)
